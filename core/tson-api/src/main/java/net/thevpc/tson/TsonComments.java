...
public class TsonComments implements Comparable<TsonComments>{
    ...
    public TsonComments concat(TsonComments other) {
        if(other!=null && !other.isBlank()){
            ...
            a.addAll(Arrays.asList(getLeadingComments()));
            a.addAll(Arrays.asList(other.getLeadingComments()));
            b.addAll(Arrays.asList(getTrailingComments()));
            b.addAll(Arrays.asList(other.getTrailingComments()));
    ...
    public int compareTo(TsonComments o) {
        int i= TsonApiUtils.compareArrays(getLeadingComments(),o.getLeadingComments());
        if(i!=0){
            return i;
        }
        i= TsonApiUtils.compareArrays(getTrailingComments(),o.getTrailingComments());
        if(i!=0){
            return i;
        }
        return 0;
